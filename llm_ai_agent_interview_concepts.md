...
这一章用于回答“你了解大模型是怎么训练出来的吗”。实习面试通常不会要求你手写训练框架，但会考察你是否理解预训练、数据、loss、显存和分布式训练这些基础概念。
...
SFT，即 Supervised Fine-Tuning，用高质量指令-回答数据继续训练模型，让模型学会遵循指令。
...
> 预训练模型只是会续写文本，SFT 让模型学会按用户指令完成任务，比如问答、总结、抽取、代码生成等。
...
2. 人类标注哪个回答更好。
3. 训练 Reward Model。
4. 用强化学习优化模型，让输出获得更高 reward。

面试答法：

> RLHF 的核心是把人类偏好转成 reward 信号，再用强化学习让模型更倾向生成被人类喜欢的回答。

### DPO

DPO 直接使用偏好数据训练模型，不需要显式训练 reward model，也不需要复杂 RL 流程。

面试答法：

> DPO 可以理解为更简单稳定的偏好优化方法，它直接让模型提高 chosen answer 的概率、降低 rejected answer 的概率。
...
LoRA 的核心思想：冻结原模型权重，只在部分线性层旁边增加低秩矩阵，让模型学习一个低秩增量。
...
其中 `W` 是原权重，`A` 和 `B` 是可训练的低秩矩阵。

优点：

- 训练参数少。
- 显存占用低。
- 多个 LoRA adapter 可以按任务切换。

面试答法：

> LoRA 不直接更新大模型主体权重，而是学习一个低秩增量，因此训练成本低、部署灵活，适合领域适配和格式适配。

### QLoRA

QLoRA 在量化模型基础上做 LoRA 微调，通常把基础模型以 4-bit 方式加载，再训练 LoRA 参数。
...
> QLoRA 降低的是训练资源门槛，但训练数据质量仍然决定最终效果。
...
推理是模型根据输入 prompt 生成输出的过程。

自回归生成：

```text
输入 tokens -> 预测下一个 token -> 拼回上下文 -> 再预测下一个 token
```

面试答法：

> LLM 推理是逐 token 生成的。每生成一个 token，都要基于已有上下文预测下一个 token，所以输出越长，延迟和成本越高。
...
> 输入长主要影响 prefill，输出长主要影响 decode。优化延迟时要分别看 prompt 长度和生成长度。

### KV Cache

KV cache 缓存历史 token 的 Key 和 Value，避免每生成一个 token 都重新计算完整上下文。
...
> KV cache 是推理加速关键。它用显存换速度，避免重复计算历史 token 的 attention key/value。
...
> 代码和结构化任务一般用低 temperature 保证稳定，创意任务可以提高 temperature 或 top-p 增加多样性。
...
> Chat 产品很关注 TTFT，因为用户希望尽快看到输出；批处理任务更关注吞吐量。
...
连续批处理会把不同时间到来的请求动态合并到 batch 中，提高 GPU 利用率。

面试答法：

> 普通 batching 要等一批请求凑齐，continuous batching 可以在生成过程中动态加入新请求，更适合 LLM 服务这种输出长度不一致的场景。

### 量化 Quantization

量化是用更低 bit 表示模型权重或激活，降低显存和加速推理。
...
> 量化是部署优化常用手段，用精度换成本和速度。要通过评估确认量化后质量是否可接受。

### 蒸馏 Distillation

蒸馏是用大模型作为 teacher，训练小模型 student 模仿它。
...
> 蒸馏能把大模型能力迁移到小模型上，但小模型上限通常受限，适合任务边界清晰的场景。

### Speculative Decoding

投机解码用小模型先生成候选 token，再由大模型验证，从而加速推理。

面试答法：

> Speculative decoding 的思想是让便宜的小模型先猜，大模型负责验证。如果小模型猜得准，就能减少大模型逐 token 生成的成本。
...
> 业务早期通常先用 API 验证价值；对数据安全、成本或延迟有更高要求时，再考虑私有化部署和推理优化。
...
> 模型选型不是越大越好，要结合任务难度、成本、延迟和稳定性。简单分类抽取可以用小模型，复杂推理和 Agent 决策再用强模型。
